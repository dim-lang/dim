...
%type <stmt> compound_statement expression_statement iteration_statement jump_statement empty_statement 
%type <stmt> matched_if_statement unmatched_if_statement other_statement
%type <stmt> statement
...
statement_list : statement { $$ = new AstStatementList(); $$->add($1); }
 ...
 /* here we use matched and unmatched statement for shift/reduce generated by if_statement */
statement : matched_if_statement { $$ = $1; }
          | unmatched_if_statement { $$ = $1; }
          ;

matched_if_statement : other_statement { $$ = $1; }
                     | T_IF T_LPAREN expression T_RPAREN matched_if_statement T_ELSE matched_if_statement { $$ = new AstIfStatement($3, $5, $7); }
                     ;

unmatched_if_statement : T_IF T_LPAREN expression T_RPAREN statement { $$ = new AstIfStatement($3, $5, nullptr); }
                       | T_IF T_LPAREN expression T_RPAREN matched_if_statement T_ELSE unmatched_if_statement { $$ = new AstIfStatement($3, $5, $7); }
                       ;

other_statement : compound_statement { $$ = $1; }
