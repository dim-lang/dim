...
void buildSemantic(Symtab *&gss, Symtab *&css, const Ast *node) {
  CASSERT(gss, "gss is null: {}", gss ? gss->toString() : "null");
  ...
  case A_DECLARATION_LIST: {
    AstDeclarationList *declList = DC(AstDeclarationList, node);
    for (int i = 0; i < declList->size(); i++) {
      CASSERT(declList->get(i), "declList#get {} is null", i);
      buildImpl(declList->get(i));
    }
