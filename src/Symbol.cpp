...
#if 0
using sptr = Symbol *;
using tsptr = TypeSymbol *;
using scptr = Scope *;

static NameGenerator ScopeNameGenerator(".");

static Scope *createImpl(Ast *ast, Scope *scope) {
  if (!ast)
    return scope;
  switch (ast->kind()) {
...
static void checkImpl(Ast *ast, Scope *scope) {
  if (!ast)
    return;
  switch (ast->kind()) {
  case AstKind::CompileUnit: {
  ...
  case AstKind::Assign: {
    ...
    LOG_ASSERT(
        variableSymbol,
        "assignee {} must already defined in scope
        {} ",
        varId->name(),
        scope->name());
  ...
  case AstKind::VarId: {
    ...
    LOG_ASSERT(
        variableSymbol,
        "variable {} must already defined in scope
        {} ",
        varId->name(),
        scope->name());
  ...
  case AstKind::Call: {
    ...
    LOG_ASSERT(
        functionSymbol,
        "function {} must already defined in scope
        {} ",
        varId->name(),
        scope->name());
